...
def report_to_text(report):
    ...
    if rec:
        ...
        lines.append(f"- Support category: {rec.get('support_category', 'neutral')}")
        lines.append(f"- Cushioning level: {rec.get('cushioning_level', 'medium')}")
        primary = rec.get("primary")
        if primary:
            lines.append(f"- Primary pick: {primary.get('brand', '')} {primary.get('name', '')}".rstrip())
        else:
            lines.append("- No matching shoes found in the database.")
        alternatives = rec.get("alternatives") or []
        if alternatives:
            lines.append("- Alternatives:")
            for shoe in alternatives:
...
# ---------------------------------------------------------------------------
# Rendered frame lookup (shown on the Streamlit results page)
# ---------------------------------------------------------------------------
def pick_rendered_images(output_base_path, n=4):
    """Pick up to n rendered frames, evenly spaced, to illustrate the analysis."""
